...
`);if(r.every(n=>n.startsWith("| "))){c.push('<div class="mdtable"><table>'+r.map(n=>"<tr>"+n.slice(2).split(" | ").map(o=>`<td>${N(o)}</td>`).join("")+"</tr>").join("")+"</table></div>");continue}for(const n of r)n.startsWith("#### ")?c.push(`<h5>${N(n.slice(5))}</h5>`):n.startsWith("### ")?c.push(`<h4>${N(n.slice(4))}</h4>`):n.startsWith("| ")?c.push('<div class="mdtable"><table><tr>'+n.slice(2).split(" | ").map(o=>`<td>${N(o)}</td>`).join("")+"</tr></table></div>"):n.startsWith("• ")?c.push(`<p class="bullet">${N(n)}</p>`):c.push(`<p>${N(n)}</p>`)}return`<div class="md">${c.join("")}</div>`}const j=e=>String(e||"").normalize("NFD").replace(/[̀-ͯ]/g,"").toLowerCase(),oa=Object.fromEntries(D),M=40,z=e=>e===0?"Truque":`${e}º círculo`,ea=()=>Math.random().toString(36).slice(2,9),k=(e,c,a,r=!1,n="",o="")=>`<details class="entry" ${r?"open":""} ${n}><summary><span class="t">${e}</span><span class="row" style="gap:.4rem;flex-wrap:nowrap"><span class="lvl-badge">${c||""}</span>${o}</span></summary><div class="body">${a}</div></details>`,F=(e,c,a="Adicionar")=>`<button class="btn small primary addbtn" ${e}="${c}" aria-label="${a}">+</button>`;function wa(e,c,a){const r=a.S,n=r.comp,o=new Set(e.build.classes.map(d=>d.classId)),l=n.classes.filter(d=>!o.has(d.id)),m=c.classesInfo.slice(1);return`
  ...
  <section class="card stack">
    ...
    <div class="stats">
      <div><b>${E(c.pb)}</b><small>Proficiência</small></div>
      <div><b>${E(c.initiative)}</b><small>Iniciativa</small></div>
  ...
  <section class="card stack"><h2>Atributos e Salvaguardas</h2>
    ...
    <div class="abil2">${D.map(([s,i])=>{const t=(e.build.saveProfs||[]).includes(s);return`<div class="abcard">
        ...
        <button class="savebtn ${t?"on":""}" data-save="${s}" aria-pressed="${t}">
          <span class="dot ${t?"on":""}"></span>Salvaguarda <b>${E(c.saves[s])}</b></button>
      </div>`}).join("")}</div>
  </section>
  ...
  <section class="card"><h2>Perícias</h2><p class="empty-note">Toque para alternar: sem proficiência → proficiente (●) → especialista (◆).</p>
    <div class="skills">${c.skills.map(s=>`<button data-skill="${s.id}" aria-pressed="${s.prof||s.exp}">
      <span class="row" style="flex-wrap:nowrap"><span class="dot ${s.exp?"exp":s.prof?"on":""}"></span>${s.nome} <small class="muted">(${s.ab.toUpperCase()})</small></span><b>${E(s.bonus)}</b></button>`).join("")}</div>
  </section>
  ...
  ${c.casters.length?`<section class="card stack">${c.casters.map(p=>`<div class="stats" style="grid-template-columns:repeat(3,1fr)">
    <div><b>${p.dc}</b><small>CD (${u(p.nome)})</small></div><div><b>${E(p.atk)}</b><small>Ataque mágico</small></div>
  ...
  ${d.length?`<section class="card slots"><h2>Espaços de magia${c.multiclass?" (combinados)":""}</h2>
    ${d.map(([p,g])=>`<div class="lvl"><span>${p}º</span>${Array.from({length:g.max},(A,C)=>`<button class="slotpip ${C<g.used?"used":""}" data-slot="${p}" data-i="${C}" aria-label="Espaço de ${p}º círculo ${C+1}${C<g.used?", gasto":""}"></button>`).join("")}</div>`).join("")}
  </section>`:""}
  ${h.length?`<section class="card slots"><h2>Magia de Pacto</h2>
    ${h.map(([p,g])=>`<div class="lvl"><span>${p}º</span>${Array.from({length:g.max},(A,C)=>`<button class="slotpip ${C<g.used?"used":""}" data-pactslot="${p}" data-i="${C}" aria-label="Espaço de pacto ${C+1}${C<g.used?", gasto":""}"></button>`).join("")}</div>`).join("")}
  </section>`:""}
  ...
  </section>`}function Ia(e,c,a){const r=c.sp,n=c.bg;return`
  ${c.classesInfo.map(o=>Ea(e,o)).join("")}
  ...
  <section class="card"><h2>Registro</h2><ul class="log">${(e.state.log||[]).map(t=>`<li>${u(t)}</li>`).join("")||"<li>—</li>"}</ul></section>`}function Qa(e,c){const a=v("#amount");a.oninput=()=>$.amount=a.value,w.querySelectorAll("[data-q]").forEach(n=>n.onclick=()=>{$.amount=n.dataset.q==="C"?"":String((Number($.amount)||0)+Number(n.dataset.q)),a.value=$.amount}),w.querySelectorAll("[data-act]").forEach(n=>n.onclick=async()=>{const o=Number($.amount);if(!o)return S("Digite um valor.");const l={resistant:v("#o-res").checked,vulnerable:v("#o-vul").checked,critical:v("#o-crit").checked};$.amount="";const m=await H($.be,$.chId,n.dataset.act,o,l,e.name);m.concentrationDC&&e.state.concentration?S(`Teste de Concentração: CD ${m.concentrationDC}`):m.log.length&&S(m.log.at(-1))}),v("#endconc")&&(v("#endconc").onclick=()=>I.save({"state.concentration":null})),v("#roll-death")&&(v("#roll-death").onclick=async()=>{const n=aa("1d20").total,o=await H($.be,$.chId,"morte",n,{},e.name);S(`d20 = ${n}. ${o.log.at(-1)??""}`)}),w.querySelectorAll("[data-rmcond]").forEach(n=>n.onclick=()=>I.save({"state.conditions":e.state.conditions.filter(o=>o!==n.dataset.rmcond)})),v("#addcond").onchange=n=>n.target.value&&I.save({"state.conditions":[...e.state.conditions,n.target.value]});const r=async(n,o)=>{const l=aa(`1d${n}`).total+c.mods.con;await o();const m=await H($.be,$.chId,"cura",Math.max(0,l),{},e.name);S(`Dado de Vida: ${l}. ${m.log[0]??""}`)};v("#short")&&(v("#short").onclick=()=>{const n=e.state.hitDice;if(n.used>=n.max)return S("Sem Dados de Vida disponíveis.");r(n.die,()=>I.save({"state.hitDice.used":n.used+1}))}),w.querySelectorAll("[data-shortclass]").forEach(n=>n.onclick=()=>{const o=e.state.hitDice.byClass,l=o.findIndex(h=>h.classId===n.dataset.shortclass),m=o[l];if(m.used>=m.max)return S("Sem Dados de Vida dessa classe.");const d=o.map((h,s)=>s===l?{...h,used:h.used+1}:h);r(m.die,()=>I.save({"state.hitDice.byClass":d,"state.hitDice.used":d.reduce((h,s)=>h+s.used,0)}))}),v("#long").onclick=async()=>{const n=Object.fromEntries(Object.entries(e.state.spellSlots||{}).map(([m,d])=>[m,{...d,used:0}])),o=Object.fromEntries(Object.entries(e.state.pactSlots||{}).map(([m,d])=>[m,{...d,used:0}])),l=(e.state.hitDice.byClass||[]).map(m=>({...m,used:0}));await I.save({"state.hp":{...e.state.hp,current:e.state.hp.max,temp:0},"state.spellSlots":n,"state.pactSlots":o,"state.hitDice":{...e.state.hitDice,byClass:l,used:0},"state.deathSaves":{success:0,fail:0},"state.conditions":e.state.conditions.filter(m=>!["inconsciente","estabilizado"].includes(m))}),S("Descanso longo concluído.")}}
