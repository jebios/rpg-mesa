...
`);if(i.every(l=>l.startsWith("| "))){o.push('<div class="mdtable"><table>'+i.map(l=>"<tr>"+l.slice(2).split(" | ").map(c=>`<td>${I(c)}</td>`).join("")+"</tr>").join("")+"</table></div>");continue}for(const l of i)l.startsWith("#### ")?o.push(`<h5>${I(l.slice(5))}</h5>`):l.startsWith("### ")?o.push(`<h4>${I(l.slice(4))}</h4>`):l.startsWith("| ")?o.push('<div class="mdtable"><table><tr>'+l.slice(2).split(" | ").map(c=>`<td>${I(c)}</td>`).join("")+"</tr></table></div>"):l.startsWith("• ")?o.push(`<p class="bullet">${I(l)}</p>`):o.push(`<p>${I(l)}</p>`)}return`<div class="md">${o.join("")}</div>`}const j=t=>String(t||"").normalize("NFD").replace(/[̀-ͯ]/g,"").toLowerCase(),na=Object.fromEntries(N),M=40,z=t=>t===0?"Truque":`${t}º círculo`,Z=()=>Math.random().toString(36).slice(2,9),w=(t,o,e,i=!1,l="",c="")=>`<details class="entry" ${i?"open":""} ${l}><summary><span class="t">${t}</span><span class="row" style="gap:.4rem;flex-wrap:nowrap"><span class="lvl-badge">${o||""}</span>${c}</span></summary><div class="body">${e}</div></details>`,F=(t,o,e="Adicionar")=>`<button class="btn small primary addbtn" ${t}="${o}" aria-label="${e}">+</button>`;function va(t,o,e){var u;const i=e.S,l=o.level,c=t.build.classes[0],n=((u=o.cls)==null?void 0:u.subclasses)||[],b=t.build.training||{};return`
  <section class="card stack">
    ...
    <div class="row"><label for="hpmax" style="margin:0">PV Máximo</label>
      <input id="hpmax" type="number" inputmode="numeric" min="1" style="width:6rem" value="${t.state.hp.max}" /></div>
  ...
  <section class="card stack"><h2>Conta</h2>
    ...
    <div class="row">${y?`${h.nivel>0?`<button class="btn small" data-prep="${h.id}">${b.includes(h.id)?"✓ Preparada":"Preparar"}</button>`:""}
         ...
         <button class="btn small" data-forget="${h.id}">Remover</button>`:""}</div>`,W=h=>[h.concentracao?"C":"",h.ritual?"R":""].filter(Boolean).map(y=>`<span class="chip">${y}</span>`).join(" ");return`
  ...
  ${u.length?`<section class="card slots"><h2>Espaços de magia</h2>
    ${u.map(([h,y])=>`<div class="lvl"><span>${h}º</span>${Array.from({length:y.max},(E,P)=>`<button class="slotpip ${P<y.used?"used":""}" data-slot="${h}" data-i="${P}" aria-label="Espaço de ${h}º círculo ${P+1}${P<y.used?", gasto":""}"></button>`).join("")}</div>`).join("")}
  </section>`:""}
  <section class="card stack"><div class="sectiontitle"><h2 style="margin:0">Minhas magias</h2><span class="lvl-badge">${b.length} preparada(s)</span></div>
    ${Object.keys(s).length?Object.entries(s).map(([h,y])=>`<h3 style="margin:.6rem 0 .2rem;color:var(--muted)">${z(Number(h))}</h3>
      <div class="stack" style="--gap:.4rem">${y.map(E=>w(`${E.nivel>0&&b.includes(E.id)?"✓ ":""}${r(E.nome)} ${W(E)}`,z(E.nivel),g(E,!0),!1,`data-k="m-${E.id}"`)).join("")}</div>`).join(""):'<p class="empty-note">Nenhuma magia ainda. Adicione pela biblioteca abaixo.</p>'}
  </section>
  ...
  ${b?`<section class="card stack"><h2>Antecedente: ${r(b.nome)}</h2>
    ...
    <b>Talento:</b> ${r(b.talento)}<br><b>Ferramenta:</b> ${r(b.ferramenta)}<br><b>Equipamento:</b> ${r(b.equipamento)}</p></section>`:""}`}function ya(t,o,e){const i=e.app,l=t.build.opcoes||[];i.querySelectorAll("[data-op]").forEach(c=>c.onclick=()=>e.save({"build.opcoes":[...l,c.dataset.op]})),i.querySelectorAll("[data-unop]").forEach(c=>c.onclick=()=>e.save({"build.opcoes":l.filter(n=>n!==c.dataset.unop)})),L(i,"classe")}const ka=["Origem","Geral","Estilo de Luta","Dádiva Épica"];function wa(t,o,e){const i=e.S,l=i.comp,c=i.ui,n=t.build.feats||[],b=j(c.featQ);let u=l.feats.filter(s=>(s.repetivel||!n.some(d=>d.id===s.id))&&(c.featCat==="all"||s.categoria===c.featCat)&&(!b||j(s.nome).includes(b)||j(s.prereq).includes(b)));const m=u.length;return u=u.slice(0,M),`
  ...
  <section class="card stack"><h2>Biblioteca de talentos</h2>
    ...
    <div class="filters">${["all",...ka].map(s=>`<button data-fcat="${s}" aria-pressed="${c.featCat===s}">${s==="all"?"Todos":s}</button>`).join("")}</div>
  ...
  <section class="card stack"><div class="sectiontitle"><h2 style="margin:0">Bolsa de moedas</h2><span class="lvl-badge">≈ ${o.coinsGP.toLocaleString("pt-BR",{maximumFractionDigits:2})} PO</span></div>
    <div class="coins">${U.map(([a,v,f])=>`<label class="coin coin-${a}"><span>${v}</span>
    ...
    <form class="row" id="coinop">
      ...
      <select id="coinkind" style="width:5.5rem" aria-label="Moeda">${U.map(([a,v])=>`<option value="${a}" ${a==="po"?"selected":""}>${v}</option>`).join("")}</select>
  ...
  <form class="stack" id="build">
    ...
    <button class="btn primary" style="width:100%">Criar ficha</button>
      ...
      <details class="entry"><summary><span class="t">Equipamento do antecedente</span></summary><div class="body">${r(s.equipamento)}</div></details>`,$("#plus2").onchange=$("#plus1").onchange=m,m(),n()}function u(){const s=l(),d={};if(!s)return d;const a=$("#plus2").value;if(a==="all")s.atributos.forEach(v=>d[v]=1);else{d[a]=2;const v=$("#plus1").value;v!==a&&(d[v]=(d[v]||0)+1)}return d}function m(){const s=u();$("#plus1wrap")&&($("#plus1wrap").style.visibility=$("#plus2").value==="all"?"hidden":""),e.querySelectorAll(".abfinal").forEach(d=>{const a=d.dataset.ab,v=Math.min(20,Number(e[a].value)+(s[a]||0));d.textContent=s[a]?`→ ${v} (${A(Y(v))})`:`(${A(Y(v))})`})}e.class.onchange=c,e.bg&&(e.bg.onchange=b),N.forEach(([s])=>e[s].oninput=m),c(),b(),m(),e.onsubmit=async s=>{var f;s.preventDefault();const d=u(),a=Object.fromEntries(N.map(([g])=>[g,Math.min(20,Number(e[g].value)+(d[g]||0))])),v=ma({name:e.name.value.trim(),ownerUid:p.user.uid,speciesId:e.species.value,classId:e.class.value,level:Number(e.level.value),abilities:a,backgroundId:((f=e.bg)==null?void 0:f.value)||null,skillProfs:[...e.querySelectorAll("[name=sk]:checked")].map(g=>g.value)},t);try{x(await p.be.createCharacter(v))}catch(g){q(g.message)}}}const oa=t=>{var l;const o=p.comp.byId.classes[t.build.classes[0].classId],e=(l=o==null?void 0:o.subclasses)==null?void 0:l.find(c=>c.id===t.build.classes[0].subclassId),i=p.comp.byId.species[t.build.speciesId];return`${(i==null?void 0:i.nome)??""} · ${(o==null?void 0:o.nome)??""} ${t.build.classes[0].level}${e?" · "+e.nome:""}`};function x(t){B(),p.chId=t,p.prevHp=null,Q.set(_(),t),p.off.push(p.be.watchCharacter(t,o=>{if(!o)return D();const e=p.prevHp;if(p.prevHp=o.state.hp.current+o.state.hp.temp,p.ch=o,K(),e!=null&&e!==p.prevHp){const i=$("#hpcard");i==null||i.classList.add(p.prevHp<e?"flash-dmg":"flash-heal"),navigator.vibrate&&p.prevHp<e&&navigator.vibrate(80)}}))}const Na=[["combate","❤️","Combate"],["ficha","📜","Ficha"],["magias","✨","Magias"],["classe","🛡️","Traços"],["talentos","⭐","Talentos"],["itens","🎒","Itens"]],C={get S(){return p},app:k,render:()=>K(),save:t=>p.be.updateCharacter(p.chId,t).catch(o=>q(o.message)),toast:q,showHome:()=>D(),confirmTwice:Ma,accountChip:ta,bindLogout:sa};function K(){const t=p.ch,o=pa(t,p.comp),e=document.activeElement,i=e==null?void 0:e.id,l=e&&"selectionStart"in e?[e.selectionStart,e.selectionEnd]:null,c=window.scrollY,n={combate:{html:Ta,bind:Da},...Ea},b=n[p.tab]||n.combate;if(k.innerHTML=`
    <header class="top">
    ...
    <nav class="tabs" role="tablist">${Na.map(([u,m,s])=>`<button role="tab" aria-selected="${p.tab===u}" data-tab="${u}"><span class="ico" aria-hidden="true">${m}</span>${s}</button>`).join("")}</nav>`,k.querySelectorAll("[data-tab]").forEach(u=>u.onclick=()=>{p.tab=u.dataset.tab,K(),window.scrollTo(0,0)}),$("#home").onclick=D,b.bind(t,o,C),window.scrollTo(0,c),i){const u=document.getElementById(i);if(u&&(u.focus({preventScroll:!0}),l&&"setSelectionRange"in u))try{u.setSelectionRange(...l)}catch{}}}function Ma(t,o){const e=$(t);if(e.dataset.armed)return!0;e.dataset.armed="1";const i=e.textContent;return e.textContent=o,setTimeout(()=>{e.isConnected&&(delete e.dataset.armed,e.textContent=i)},3e3),!1}function Ta(t,o){const{hp:e,deathSaves:i={success:0,fail:0},conditions:l=[],dead:c}=t.state,n=e.max+e.temp,b=Math.round(e.current/n*100),u=Math.round(e.temp/n*100),m=e.current/e.max>.5?"":e.current/e.max>.25?"mid":"low",s=e.current===0&&!c,d=t.state.concentration?p.comp.byId.spells[t.state.concentration]:null;return`
  ...
  <section class="card stack"><h2>Condições</h2>
    ...
    <div class="row"><select id="addcond" class="grow" aria-label="Adicionar condição"><option value="">Adicionar condição…</option>
      ${ba.filter(a=>!l.includes(a)).map(a=>`<option value="${a}">${J[a]||a}</option>`).join("")}</select></div>
  </section>
