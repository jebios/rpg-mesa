import"./pwa-DNJwNots.js";import{e as p,t as f,$ as b,u as k,g as F,r as U,l as z,s as q,a as O,b as H,v as V,p as G,d as _,c as K,f as S,C as L,A as Q,h as J,i as Y}from"./auth-DIWDf66d.js";let C;async function W(){if(C)return C;try{const e=new URL("data/",document.baseURI);C=(await(await fetch(new URL("itens-comuns.json",e))).json()).itens||[]}catch{C=[]}return C}const X=[["jogadores","🧑‍🤝‍🧑","Jogadores"],["combate","⚔️","Combate"],["npcs","🎭","NPCs"],["historia","📖","História"],["itens","🎒","Itens"],["musica","🎵","Música"]],T={};function D(e,a,i=600){clearTimeout(T[e]),T[e]=setTimeout(a,i)}const Z=()=>Math.floor(Math.random()*20)+1,x=e=>e.replace(":","_"),I=e=>String(e||"").normalize("NFD").replace(/[̀-ͯ]/g,"").toLowerCase();function aa(e){const a=String(e||"").match(/(?:youtu\.be\/|youtube\.com\/(?:watch\?v=|embed\/|shorts\/))([\w-]{11})/);return a?a[1]:null}function ea(e){const a=String(e||"").match(/open\.spotify\.com\/(track|album|playlist|artist)\/([a-zA-Z0-9]+)/);return a?`https://open.spotify.com/embed/${a[1]}/${a[2]}`:null}const ta=e=>/\.(mp3|ogg|wav|m4a|flac)(\?.*)?$/i.test(String(e||""));function sa(e){const a=aa(e);if(a)return{tipo:"youtube",videoId:a};const i=ea(e);return i?{tipo:"spotify",embedUrl:i}:ta(e)?{tipo:"audio"}:{tipo:"link"}}function oa(e){const{S:a}=e,i=a.combat||{ordem:[],turno:0,rodada:1},r=i.ordem||[],s=o=>{if(o.startsWith("pc:")){const c=a.chars.find(v=>v.id===o.slice(3));return c?{nome:c.name,tipo:"pc",ca:c.build.ac??10,pv:c.state.hp.current,pvMax:c.state.hp.max}:null}const d=a.npcs.find(c=>c.id===o.slice(4));return d?{nome:d.nome,tipo:"npc",ca:d.ca??"—",pv:d.pvAtual??0,pvMax:d.pvMax??0}:null},n=new Set(r.map(o=>o.ref)),l=[...a.chars.filter(o=>!n.has("pc:"+o.id)).map(o=>({ref:"pc:"+o.id,nome:o.name,tipo:"pc"})),...a.npcs.filter(o=>o.tipo==="combate"&&!n.has("npc:"+o.id)).map(o=>({ref:"npc:"+o.id,nome:o.nome,tipo:"npc"}))],m=[...r].map((o,d)=>({...o,i:d,r:s(o.ref)})).filter(o=>o.r).sort((o,d)=>d.iniciativa-o.iniciativa);return`<div class="stack">
  ...
  </div>`}function ia(e){const{S:a,app:i,render:r}=e,s=()=>a.combat||{ordem:[],turno:0,rodada:1},n=o=>e.saveCombat({...s(),...o});i.querySelectorAll("[data-roll]").forEach(o=>o.onclick=()=>{const d=document.getElementById("add-"+x(o.dataset.roll));d&&(d.value=Z())}),i.querySelectorAll("[data-addinit]").forEach(o=>o.onclick=()=>{const d=o.dataset.addinit,c=document.getElementById("add-"+x(d)),v=Number(c==null?void 0:c.value);if(!v&&v!==0)return f("Digite ou role a iniciativa.");const u=s();n({ordem:[...u.ordem||[],{ref:d,iniciativa:v}]})}),i.querySelectorAll("[data-rminit]").forEach(o=>o.onclick=()=>{const d=s();n({ordem:(d.ordem||[]).filter(c=>c.ref!==o.dataset.rminit)})}),i.querySelectorAll("[data-initval]").forEach(o=>o.onchange=()=>{const d=s();n({ordem:(d.ordem||[]).map(c=>c.ref===o.dataset.initval?{...c,iniciativa:Number(o.value)||0}:c)})}),i.querySelectorAll("[data-npchp]").forEach(o=>o.onchange=()=>{e.updateNpc(o.dataset.npchp,{pvAtual:Math.max(0,Number(o.value)||0)})});const l=s(),m=(l.ordem||[]).length;b("#c-prev")&&(b("#c-prev").onclick=()=>{let o=(l.turno||0)-1,d=l.rodada||1;o<0&&(o=m-1,d=Math.max(1,d-1)),n({turno:o,rodada:d})}),b("#c-next")&&(b("#c-next").onclick=()=>{let o=(l.turno||0)+1,d=l.rodada||1;o>=m&&(o=0,d+=1),n({turno:o,rodada:d})}),b("#c-end")&&(b("#c-end").onclick=()=>{confirm("Encerrar o combate e limpar a iniciativa?")&&n({ordem:[],turno:0,rodada:1})})}function na(e){const{S:a}=e,i=s=>a.npcs.filter(n=>n.tipo===s),r=s=>`<details class="entry" data-k="npc-${s.id}" ${a.openNpc.has(s.id)?"open":""}>
    ...
    </div></details>`,s=I(a.comunsQ||""),n=a.comunsCat||"all",l=(a.itensComuns||[]).filter(c=>(n==="all"||c.categoria===n)&&(!s||I(c.nome).includes(s))),m=new Set(i.map(c=>w(c.nome))),o=c=>{const v=m.has(w(c.nome));return`<div class="libitem">
      <div class="info"><span>${p(c.nome)}</span>
        <small>${[c.categoria,c.peso,c.preco].filter(Boolean).join(" · ")}${c.descricao?" — "+p(c.descricao):""}</small></div>
      <button class="btn small ${v?"":"primary"}" data-addcommon="${w(c.nome)}" ${v?'disabled title="Já está na sua lista"':""}>${v?"✓":"+"}</button>
    ...
    ${s.length?s.map(m=>`<div class="card stack"><h2>${p(m)}</h2>
  ...
  </div>`}function ba(e){const{S:a,app:i,render:r}=e;b("#musnew").onsubmit=async s=>{var c;s.preventDefault();const n=b("#mus-nome").value.trim(),l=b("#mus-url").value.trim(),m=b("#mus-cat").value.trim()||"Livre";if(!n||!l)return;const o={id:k(),nome:n,categoria:m,url:l,...sa(l)};if(o.tipo==="link"&&!confirm("Não consegui reconhecer este link como YouTube, Spotify ou áudio direto. Adicionar mesmo assim? (vai abrir em outra aba ao tocar)"))return;const d=[...((c=a.music)==null?void 0:c.faixas)||[],o];try{await e.saveDmDoc("musicas",{faixas:d}),f("Música adicionada.")}catch(v){f(v.message)}},i.querySelectorAll("[data-play]").forEach(s=>s.onclick=()=>{var l;const n=(((l=a.music)==null?void 0:l.faixas)||[]).find(m=>m.id===s.dataset.play);if(n){if(n.tipo==="link"){window.open(n.url,"_blank","noopener");return}a.nowPlaying=n,r()}}),b("#m-stop")&&(b("#m-stop").onclick=()=>{a.nowPlaying=null,r()}),i.querySelectorAll("[data-delmus]").forEach(s=>s.onclick=()=>{var l,m;const n=(((l=a.music)==null?void 0:l.faixas)||[]).filter(o=>o.id!==s.dataset.delmus);((m=a.nowPlaying)==null?void 0:m.id)===s.dataset.delmus&&(a.nowPlaying=null),e.saveDmDoc("musicas",{faixas:n}).catch(o=>f(o.message))})}const h=b("#app"),t={be:null,comp:null,itensComuns:[],user:null,cid:null,camp:null,chars:[],npcs:[],story:null,combat:null,itemLib:null,music:null,nowPlaying:null,tab:"jogadores",open:new Set,openNpc:new Set,inputs:{},off:[],comunsQ:"",comunsCat:"all"},M=()=>`rpgmesa:dm-last:${t.user.uid}`,P=()=>{t.off.forEach(e=>e()),t.off=[]};va().catch(e=>{h.innerHTML=`<div class="shell"><div class="card">Erro ao iniciar: ${p(e.message)}</div></div>`});async function va(){t.be=await F(),U(t.be,h,{title:"Escudo do Mestre",subtitle:"Entre com sua conta de Mestre para gerenciar suas campanhas.",onSignedOut:()=>{P(),t.camp=null,t.cid=null,t.chars=[],t.nowPlaying=null,A()}},async e=>{t.user=e,h.innerHTML='<p class="muted" style="padding:1rem">Carregando compêndio…</p>',[t.comp,t.itensComuns]=await Promise.all([z(t.be),W()]);const a=q.get(M());a?E(a):N()})}const R=()=>t.be.mode==="local"?'<p class="banner">Modo demo: dados salvos neste navegador. Abra o Grimório em outra aba (com outra conta) para ver a sincronização.</p>':"";function N(){P(),t.camp=null,t.cid=null,t.nowPlaying=null,A(),q.set(M(),null),t.off.push(t.be.watchMyCampaigns(e=>{var i;const a=((i=b("#cname"))==null?void 0:i.value)||"";e.sort((r,s)=>r.name.localeCompare(s.name)),h.innerHTML=`<div class="shell stack" style="max-width:640px">
      ...
      <form class="card stack" id="f"><h2>Nova campanha</h2>
        ...
        <button class="btn primary">Criar sala</button></form></div>`,H(t.be,h),h.querySelectorAll("[data-open]").forEach(r=>r.onclick=()=>E(r.dataset.open)),b("#compfile").onchange=async r=>{const s=r.target.files[0];if(s)try{const n=JSON.parse(await s.text()),l=V(n);f("Importando compêndio…"),await t.be.saveCompendium(n),t.comp=await z(t.be,{force:!0}),f(`Compêndio importado: ${l.magias} magias, ${l.talentos} talentos, ${l.classes} classes.`),N()}catch(n){f(/permission|insufficient/i.test(n.message)?"Sem permissão: só a conta de administrador pode importar o compêndio.":"Erro: "+n.message)}},b("#f").onsubmit=async r=>{r.preventDefault();try{const{id:s}=await t.be.createCampaign(b("#cname").value.trim());E(s)}catch(s){f(s.message)}}}))}function E(e){P(),t.cid=e,t.camp=null,t.chars=[],t.npcs=[],t.story=null,t.combat=null,t.itemLib=null,t.music=null,t.nowPlaying=null,t.tab="jogadores",q.set(M(),e),t.off.push(t.be.watchCampaign(e,a=>{if(!a||a.dmUid!==t.user.uid)return N();t.camp=a,y()})),t.off.push(t.be.watchPartyCharacters(e,a=>{const i=Object.fromEntries(t.chars.map(r=>[r.id,r.state.hp.current+r.state.hp.temp]));t.chars=a.sort((r,s)=>r.name.localeCompare(s.name)),y(),a.forEach(r=>{var l;const s=i[r.id],n=r.state.hp.current+r.state.hp.temp;s!=null&&s!==n&&((l=document.querySelector(`[data-pc="${r.id}"]`))==null||l.classList.add(n<s?"flash-dmg":"flash-heal"))})})),t.off.push(t.be.watchNpcs(e,a=>{t.npcs=a.sort((i,r)=>i.nome.localeCompare(r.nome)),y()})),t.off.push(t.be.watchDmDoc(e,"notas",a=>{t.story=a||{texto:""},y()})),t.off.push(t.be.watchDmDoc(e,"itens",a=>{t.itemLib=a||{itens:[]},y()})),t.off.push(t.be.watchCombat(e,a=>{t.combat=a||{ordem:[],turno:0,rodada:1},y()})),t.off.push(t.be.watchDmDoc(e,"musicas",a=>{t.music=a||{faixas:[]},y()}))}const g={get S(){return t},app:h,render:()=>y(),toast:f,createNpc:e=>t.be.createNpc(t.cid,e),updateNpc:(e,a)=>t.be.updateNpc(t.cid,e,a),deleteNpc:e=>t.be.deleteNpc(t.cid,e),saveDmDoc:(e,a)=>t.be.saveDmDoc(t.cid,e,a),saveCombat:e=>t.be.saveCombat(t.cid,e)};let j=null;function fa(){let e=document.getElementById("music-bar");return e||(e=document.createElement("div"),e.id="music-bar",document.body.appendChild(e)),e}function A(){const e=fa(),a=t.nowPlaying;if(!a){e.classList.remove("show"),e.innerHTML="",j=null;return}if(j===a.id)return;j=a.id,e.classList.add("show");let i="";a.tipo==="youtube"?i=`<iframe src="https://www.youtube.com/embed/${a.videoId}?autoplay=1&controls=1" allow="autoplay" style="width:200px;height:60px;border:0;border-radius:8px" title="Player"></iframe>`:a.tipo==="spotify"?i=`<iframe src="${p(a.embedUrl)}?utm_source=generator" width="260" height="80" style="border:0;border-radius:8px" allow="autoplay; encrypted-media" title="Player"></iframe>`:a.tipo==="audio"&&(i=`<audio src="${p(a.url)}" autoplay loop controls style="width:220px"></audio>`),e.innerHTML=`<div class="row" style="align-items:center;gap:.6rem;flex-wrap:nowrap">
    ...
    ${i}<button class="btn small danger" id="mb-stop">✕</button></div>`,b("#mb-stop",e).onclick=()=>{t.nowPlaying=null,y()}}function y(){if(!t.camp){A();return}const e=Object.values(t.camp.members||{}).filter(a=>a==="player").length;G(()=>{h.innerHTML=`<div class="shell">
      <div class="topbar">
      ...
      ${ha()}
    </div>`,b("#copy").onclick=()=>{var a;return(a=navigator.clipboard)==null?void 0:a.writeText(t.camp.joinCode).then(()=>f("Código copiado."))},b("#back").onclick=N,H(t.be,h),h.querySelectorAll("[data-tab]").forEach(a=>a.onclick=()=>{t.tab=a.dataset.tab,y()}),$a()}),A()}function ha(){return t.tab==="combate"?oa(g):t.tab==="npcs"?na(g):t.tab==="historia"?ca(g):t.tab==="itens"?ma(g):t.tab==="musica"?ua(g):t.chars.length?`<div class="party">${t.chars.map(ya).join("")}</div>`:`<div class="card empty"><h2>Esperando os aventureiros…</h2><p class="muted">Peça para cada jogador entrar no Grimório com a própria conta, abrir o personagem e, na aba <b>Ficha → Mesa</b>, digitar o código <b>${p(t.camp.joinCode)}</b>.</p></div>`}function $a(){if(t.tab==="combate")return ia(g);if(t.tab==="npcs")return ra(g);if(t.tab==="historia")return la(g);if(t.tab==="itens")return pa(g);if(t.tab==="musica")return ba(g);ga()}function ya(e){var v;const a=_(e,t.comp),{hp:i,conditions:r=[],dead:s,deathSaves:n={}}=e.state,l=i.max+i.temp,m=i.current/i.max,o=m>.5?"":m>.25?"mid":"low",d=Object.entries(e.state.spellSlots||{}),c=`${((v=a.sp)==null?void 0:v.nome)??""} · ${K(e.build,t.comp)}`;return`<article class="card pc stack ${s?"dead":i.current===0?"down":""}" data-pc="${e.id}">
    <div><h3>${p(e.name)}</h3><div class="meta">${p(c)} · jogador: ${p(e.ownerName||"—")}</div></div>
    ...
    ${r.length?`<div class="row">${r.map(u=>`<button class="chip bad" data-rmcond="${u}" data-id="${e.id}" title="Remover">${L[u]||u} ✕</button>`).join("")}</div>`:""}
    ...
    <details class="more" data-more="${e.id}" ${t.open.has(e.id)?"open":""}><summary>Ficha completa</summary>
      <div class="stack" style="margin-top:.5rem">
        ...
        <div class="abil">${Q.map(([u,$])=>`<div><small>${$.slice(0,3)}</small><b>${S(a.mods[u])}</b><small>${e.build.abilities[u]} · TR ${S(a.saves[u])}</small></div>`).join("")}</div>
        ...
        <select data-addcond="${e.id}" aria-label="Aplicar condição"><option value="">Aplicar condição…</option>
          ${J.filter(u=>!r.includes(u)).map(u=>`<option value="${u}">${L[u]||u}</option>`).join("")}</select>
